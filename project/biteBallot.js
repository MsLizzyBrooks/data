...
function preload() {
  // Use fetch to get the lunch menu from the Firebase function
  loadLunchMenu().then(data => {
    currentLunch = data;
  }).catch(error => {
    console.error('Error fetching lunch menu:', error);
    currentLunch = 'Failed to load lunch menu';
  });
}
...
async function loadLunchMenu() {
  const functionUrl = 'https://us-central1-biteballot-e3be3.cloudfunctions.net/getLunchMenu'; // Use your actual function URL
  ...
  try {
    ...
    console.error('Error fetching lunch menu:', error);
    return 'Failed to load lunch menu';
  }
}
